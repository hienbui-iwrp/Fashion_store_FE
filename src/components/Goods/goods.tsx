...
export const Goods = (props: { role?: number }) => {
      ...
      <Space direction='vertical' style={{ width: '99%' }} size='large'>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <AddButton
            label='Thêm mới'
            onClick={() => {
              if (props?.role == 5) router.push(Routes.goodsManager.goodsDetail)
              else router.push(Routes.admin.goodsDetail)
            }}
            large
          />
          <InputSearch
        ...
        <TableList<GoodsProps>
          ...
          selectUrl={
            props?.role == 5
              ? Routes.goodsManager.goodsDetail
